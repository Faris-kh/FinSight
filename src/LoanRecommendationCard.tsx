  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 h-8 bg-slate-700 rounded-md relative overflow-hidden">
        <div
          className={`h-full rounded-md transition-all duration-500 ease-out ${
            c.binds
              ? 'bg-rose-950 ring-2 ring-inset ring-rose-700'
              : 'bg-indigo-950 ring-1 ring-inset ring-indigo-800'
          }`}
          style={{ width: `${pct}%`, minWidth: c.value > 0 ? '4px' : '0' }}
        />
        {/* Tick guides at 33% and 66% */}
        {[33.3, 66.6].map(tick => (
          <div
            key={tick}
            className="absolute top-0 h-full w-px bg-slate-600 opacity-50 pointer-events-none"
            style={{ left: `${tick}%` }}
          />
        ))}
      </div>
      ...
      <div className="w-24 flex-shrink-0 text-right">
        <span className={`text-xs font-bold tabular-nums ${c.binds ? 'text-rose-300' : 'text-slate-300'}`}>
    ...
    <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-slate-700">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <span className="text-[2.5rem] leading-none font-extrabold text-white tracking-tight">
            {negativeEquity ? 'Manual Review Required' : formatSAR(maxLoan)}
          </span>
          {!negativeEquity && (
            <span className="mb-0.5 flex items-center gap-1.5 px-2.5 py-1 bg-slate-700 border border-slate-600 rounded-full text-xs font-semibold text-slate-300">
      ...
      <div className="px-6 py-5 border-b border-slate-700">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] text-slate-500 font-medium">X-axis max: SAR 15M</span>
        </div>
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex justify-between mt-1" style={{ paddingLeft: '7.5rem', paddingRight: '10rem' }}>
              {['SAR 0', 'SAR 5M', 'SAR 10M', 'SAR 15M'].map(label => (
                <span key={label} className="text-[10px] text-slate-600">{label}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="bg-slate-900 rounded-xl px-5 py-3">
          ...
          <div className="flex justify-between mt-2 px-8">
            {[12, 18, 24, 30, 36, 42, 48, 54, 60].map(v => (
              <span
                key={v}
                className={`text-[10px] tabular-nums ${tenor === v ? 'text-indigo-400 font-bold' : 'text-slate-600'}`}
              >
                {v}
              </span>
            ))}
        ...
        <div className="bg-slate-900 rounded-xl px-5 py-3">
          ...
          <div className="flex justify-between mt-2 px-8">
            {[4, 6, 8, 10, 12, 14].map(v => (
              <span
                key={v}
                className={`text-[10px] tabular-nums ${Math.abs(profitRate - v) < 0.125 ? 'text-indigo-400 font-bold' : 'text-slate-600'}`}
              >
                {v}%
              </span>
            ))}
        ...
        <p className="text-[11px] text-slate-600 pt-2 leading-relaxed">
          ...
          <span className="text-slate-500 font-semibold">Debt/EBITDA ≤ 4×</span> (earnings),{' '}
          <span className="text-slate-500 font-semibold">D/E ≤ 2.5×</span> (equity), and{' '}

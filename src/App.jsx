...
export default function FinSightApp() {
  ...
  const handleFileChange = async (e) => {
    ...
    if (selectedFile) {
      ...
      // Read the file as text to parse CSV
      ...
      reader.onload = (event) => {
        ...
        // Second row is data (we'll use first data row)
      ...
      // For Excel files (.xlsx), we'd need a library like SheetJS
      // For now, we support CSV which can be read as text
      if (selectedFile.name.endsWith('.csv') || selectedFile.type === 'text/csv') {
        reader.readAsText(selectedFile);
      } else {
        // For non-CSV, show message
  ...
  const autoMapFields = (columns) => {
    ...
    // More specific rules listed first to prevent broad keywords stealing columns
    // Order matters: currentLiabilities must be checked before currentAssets
    // totalAssets must be checked before totalDebt
    const matchRules = [
    ...
    // Track which columns have already been claimed so no two fields share a column
    const usedColumns = new Set();

    matchRules.forEach(({ field, keywords }) => {
      if (mappings[field]) return; // already mapped, skip

      for (const col of columns) {
        if (usedColumns.has(col)) continue; // column already taken by another field
 ...
 const processDataWithMappings = () => {
    ...
    // 1. Initialize Accumulators for Income Statement (SUM Rule)
    let sumRevenue = 0;
    let sumExpenses = 0;
    let sumCashFlow = 0;

    // Array for the Real Historical Charting
    const monthlyRevenue = [];

    // Attempt to automatically find the 'month' or 'date' column for the chart X-axis
    ...
    rows.forEach((row, idx) => {
      // Helper to extract and parse numbers safely
      ...
      // Aggregate Income Statement items
      sumRevenue += rowRev;
      sumExpenses += rowExp;
      sumCashFlow += rowCF;

      // Determine label for the X-axis
      const monthLabel = monthColName && row[monthColName] 
        ? row[monthColName] 
        : `Month ${idx + 1}`;

      // Calculate missing row-level cashflow for chart visualization if needed
      if (!rowCF && rowRev > 0) {
          rowCF = (rowRev - (rowExp || (rowRev * 0.75))) * 0.8;
      }

      // Populate actual history for the AreaChart
      monthlyRevenue.push({
        month: monthLabel,
        revenue: rowRev,
        expenses: rowExp || (rowRev * 0.75), // Chart fallback
        profit: rowRev - (rowExp || (rowRev * 0.75)),
        cashFlow: rowCF
      });
    });

    // 2. Annualization for the ML Model
    // If less than 12 months, scale it up to a 12-month run-rate
    ...
    // 3. Balance Sheet Extraction (LATEST Rule)
    // Extract values ONLY from the very last row (the most recent financial position)
    ...
    // Applied to the newly aggregated and annualized figures
    ...
    // Smart equity/debt calculation
    if (!equity && totalAssets && totalDebt) {
      equity = totalAssets - totalDebt;
    } else if (!totalDebt && totalAssets && equity) {
      totalDebt = totalAssets - equity;
    } else if (!equity && !totalDebt) {
      equity = totalAssets * 0.6;
      totalDebt = totalAssets - equity;
    } else if (!equity) {
      equity = totalAssets * 0.6;
    } else if (!totalDebt) {
    ...
    // Extract Company Name from mapping or file name
    ...
    setFinancialData({
      ...
      monthlyRevenue, // Now uses the real historical data loop
      assetBreakdown: [
  ...
  const runForecast = async () => {
    ...
    try {
      // Send your financial data to the FastAPI backend
      ...
      // Parse the array of 6 months the backend sends back
      const forecast = await response.json();

      // Grab the last historical point to use as the anchor
      const historicalData = financialData.monthlyRevenue;
      const lastHistorical = historicalData[historicalData.length - 1];
      const combined = [
        ...historicalData.map(d => ({ month: d.month, actualCashFlow: d.cashFlow })),
        // Anchor point: shares the last historical month, carries both line values
        // so Recharts draws both lines through the same coordinate
        {
      ...
      // Calculate summary stats from the 6 returned months
      ...
      // Determine trend from first vs last forecasted month
      ...
      // If average forecast is negative, company is in decline regardless
      // of whether losses are shrinking or growing
      let trend;
      if (avg < 0) {
        trend = 'Declining';
      } else if (change > 0.03) {
      ...
      // This triggers if the backend is not running at all
  ...
  //   SCENARIO Analysis (doesnt touch real data just simulates scenarios)
  ...
  const computeAssessment = (data) => {
    ...
    const totalWeight = thresholds.currentRatio.weight + thresholds.debtToEquity.weight + thresholds.profitMargin.weight + thresholds.roa.weight + thresholds.dscr.weight;
    const weightedScore = (
    ...
    //  (Assessment Logic)
    //  Calculates all 5 financial ratios
  ...
  const getTotalWeight = () => {
    return (
      thresholds.currentRatio.weight + 
      thresholds.debtToEquity.weight + 
      thresholds.profitMargin.weight + 
      thresholds.roa.weight + 
      thresholds.dscr.weight
    );
  };
          ...
          <div className="flex items-center gap-3">
            {portfolio.length > 0 && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg text-sm font-medium transition-colors"
              >
            ...
            <button
              onClick={() => { localStorage.removeItem('finsight_auth'); window.location.href = '/'; }}
        ...
        <main className="w-full px-8 py-10 max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              <label
                htmlFor="file"
                className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 hover:border-indigo-400 rounded-xl p-12 cursor-pointer transition-colors group"
              >
              ...
              <button
                ...
                className="w-full mt-4 px-4 py-3 border border-slate-300 hover:border-indigo-400 hover:bg-indigo-50 text-slate-600 hover:text-indigo-700 rounded-lg text-sm font-semibold transition-colors"
              >
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) => handleManualDataChange('companyName', e.target.value)}
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none placeholder-slate-400"
                  ...
                  {[
                    ...
                    <div key={field}>
                      ...
                      <input
                        ...
                        onChange={(e) => handleManualDataChange(field, e.target.value)}
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none placeholder-slate-400"
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 bg-slate-900 hover:bg-slate-800 text-white py-3 rounded-lg text-sm font-semibold transition-colors"
                  >
                    Submit Data
                  </button>
                  <button
                    onClick={() => setShowManualEntry(false)}
                    className="px-6 py-3 border border-slate-300 hover:bg-slate-50 text-slate-600 rounded-lg text-sm font-semibold transition-colors"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <div className="px-6 py-4 border-b border-indigo-100 bg-indigo-50 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-indigo-600 mt-0.5">
                    {detectedColumns.length} columns detected. Auto-mapping applied — review and adjust if needed.
                  </p>
                </div>
              </div>
              <div className="p-6">
                  ...
                  {[
                    ...
                    <div key={field}>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none"
                      >
                      ...
                      {fieldMappings[field] && (
                        <p className="text-xs text-emerald-600 mt-1">✓ {fieldMappings[field]}</p>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    onClick={processDataWithMappings}
                    className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-lg text-sm font-semibold transition-colors"
                  >
            ...
            <div className="bg-white border border-emerald-200 rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="p-5">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white py-3.5 rounded-lg text-sm font-semibold transition-colors"
                >
    ...
    return (
      <div className="min-h-screen bg-slate-50">
        ...
        <header className="bg-slate-900 px-8 py-4 flex items-center justify-between sticky top-0 z-10 shadow-md">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="FinSight" className="h-20 w-auto" />
            <span className="ml-4 text-slate-400 text-sm font-medium border-l border-slate-700 pl-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => { setCurrentPage('upload'); setFinancialData(null); setUploadedFile(null); setAssessmentResults(null); setForecastData(null); }}
              className="flex items-center gap-2 px-4 py-2 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg text-sm font-medium transition-colors"
            >
        ...
        <main className="w-full px-8 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                  financialData.revenue > financialData.expenses
                    ? 'bg-emerald-100 text-emerald-600'
                    : 'bg-rose-100 text-rose-600'
                }`}>
              ...
              <p className="text-2xl font-bold text-slate-900">
                {((financialData.revenue - financialData.expenses) / 1000000).toFixed(2)}M
              </p>
              <p className="text-xs text-slate-400 mt-1">SAR — Annualised</p>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                  financialData.currentAssets / financialData.currentLiabilities >= thresholds.currentRatio.min
                    ? 'bg-emerald-100 text-emerald-600'
                    : 'bg-rose-100 text-rose-600'
                }`}>
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                  financialData.equity <= 0
                    ? 'bg-rose-100 text-rose-600'
                    : financialData.totalDebt / financialData.equity <= thresholds.debtToEquity.max
                      ? 'bg-emerald-100 text-emerald-600'
                      : 'bg-rose-100 text-rose-600'
                }`}>
              ...
              <p className="text-2xl font-bold text-slate-900">
                {financialData.equity <= 0 ? 'N/A' : (financialData.totalDebt / financialData.equity).toFixed(2)}
              </p>
          ...
          <div className="bg-slate-900 border-2 border-slate-800 rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-lg text-sm font-semibold transition-colors"
              >
            ...
            {forecastData && (
              <div className="space-y-5">
                ...
                <div className="h-72 bg-white rounded-xl p-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={forecastData.combined}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: 'white', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '12px', color: '#1E293B' }} formatter={(value, name) => [
                      ...
                      {/* Vertical reference line at the forecast boundary */}
                ...
                <div className="flex items-center gap-6 mt-4 justify-center">
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    <div className="w-5 h-0.5 bg-indigo-500 rounded" style={{ backgroundImage: 'repeating-linear-gradient(90deg, #6366F1 0px, #6366F1 4px, transparent 4px, transparent 8px)' }} />
                    Forecast
                  </div>
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    <div className="w-5 h-3 bg-indigo-500 rounded opacity-20" />
                    Confidence Band
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            {/* Radar + Debt vs Equity side by side */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              {(() => {
                ...
                const radarData = [
                  ...
                  { metric: 'Debt / Equity', score: clamp(4 - de,  0, 4),   fullMark: 100 },
          ...
          <div className="flex items-center justify-between pb-4 bg-white border border-slate-200 rounded-xl shadow-sm px-6 py-4">
            <div className="flex items-center gap-4">
              <div>
                ...
                <select
                  ...
                  className="px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 font-medium focus:ring-2 focus:ring-indigo-500 outline-none"
                >
            ...
            <button
              ...
              className="flex items-center gap-2 px-8 py-3.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-semibold text-sm shadow-md transition-colors"
            >
              <Calculator className="h-5 w-5" />
              Run Funding Assessment
            </button>
          </div>
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <header className="bg-slate-900 px-8 py-4 flex items-center justify-between sticky top-0 z-10 shadow-md">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="FinSight" className="h-20 w-auto" />
            <span className="ml-4 text-slate-400 text-sm font-medium border-l border-slate-700 pl-4">
              Assessment Report — {financialData.companyName}
            </span>
          </div>
          <div className="flex items-center gap-3">
            ...
            >
              <Zap className="h-4 w-4" />
              {isScenarioMode ? 'Exit Simulation' : 'Scenario Analysis Mode'}
            </button>
            ...
            <button onClick={() => setShowSettings(!showSettings)} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-semibold transition-colors">
              ...
              {showSettings ? 'Hide Config' : 'Configure'}
            </button>
        ...
        <main className="w-full px-8 py-6 space-y-6">
          ...
          {showSettings && (
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={resetThresholds} className="text-xs text-slate-500 hover:text-slate-800 font-semibold px-3 py-1.5 hover:bg-slate-100 rounded-lg transition-colors">
                  Reset to Defaults
                </button>
              </div>
                ...
                {[
                  ...
                  <div key={key} className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                    ...
                    <input
                      ...
                      onChange={(e) => handleThresholdChange(key, field, e.target.value)}
                      className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 mb-3"
                    />
                    <div className="flex items-center justify-between text-xs text-slate-500 mb-1">
                      <span>Weight</span>
                      <span className="font-bold text-slate-700">{thresholds[key].weight}%</span>
                    </div>
                    <input
                      type="range" min="0" max="100"
                      value={thresholds[key].weight}
                      onChange={(e) => handleThresholdChange(key, 'weight', e.target.value)}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-lg font-bold ${getTotalWeight() === 100 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {getTotalWeight()}%
                  </span>
                  {getTotalWeight() !== 100 && (
                    <span className="text-xs text-rose-500">Must equal 100%</span>
                  )}
                </div>
                <button
                  onClick={() => { setShowSettings(false); calculateAssessment(); }}
                  className="flex items-center gap-2 px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-sm font-semibold transition-colors"
                >
                  ...
                  <div key={key}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs font-bold text-slate-800">
                        {scenarioData[key] >= 1000000
                          ? `${(scenarioData[key] / 1000000).toFixed(2)}M`
                          : `${(scenarioData[key] / 1000).toFixed(0)}K`} SAR
                      </span>
                    </div>
                    <input
                      type="range"
                      ...
                      max={max}
                      step={(max - min) / 200}
                      value={scenarioData[key]}
                      onChange={(e) => {
                        const updated = {
                          ...scenarioData,
                          [key]: parseFloat(e.target.value)
                        };
                        // Cash flow dynamically updates as Revenue - Expenses
                    ...
                    <input
                      type="number"
                      value={Math.round(scenarioData[key])}
                      onChange={(e) => {
                        const updated = {
                          ...scenarioData,
          ...
          {/* ── Compute active results — scenario or real ── */}
          {(() => {
            const activeData    = isScenarioMode && scenarioData ? scenarioData : financialData;
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-wide">
                          {isScenarioMode ? 'Scenario Decision' : 'Funding Decision'}
                        </span>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        <div key={label} className={`flex items-center justify-between px-4 py-3 rounded-lg border ${
                          ...
                          score >= 60 ? 'bg-amber-50 border-amber-100' :
                          ...
                          <div className="text-right">
                            ...
                            <p className={`text-xs font-bold ${score >= 80 ? 'text-emerald-600' : score >= 60 ? 'text-amber-600' : 'text-rose-600'}`}>
                              {score} / 100
                            </p>
                          </div>
                        </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-100 to-white p-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            ...
            <button
              onClick={() => { setCurrentPage('upload'); setFinancialData(null); setUploadedFile(null); setAssessmentResults(null); setForecastData(null); }}
              className="flex items-center gap-2 px-5 py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800"
            >
                          ...
                          <td className="px-6 py-4">
                            ...
                            {entry.knockouts > 0 && (
                              <div className="text-xs text-red-600 font-medium mt-0.5">
                                {entry.knockouts} disqualifier{entry.knockouts > 1 ? 's' : ''}
                              </div>
                            )}
                          </td>
                          <td className="px-6 py-4 text-sm text-slate-600">{entry.assessedAt}</td>
                          <td className="px-6 py-4 text-sm font-medium text-slate-900">
                            {(entry.revenue / 1000000).toFixed(1)}M SAR
                          </td>
                          <td className="px-6 py-4">
                            <span className={`text-lg font-bold 
                              ${parseFloat(entry.overallScore) >= 70 ? 'text-emerald-600' : 
                                parseFloat(entry.overallScore) >= 50 ? 'text-amber-600' : 'text-red-600'}`}>
                              {entry.overallScore}%
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold
                              ${entry.decision === 'APPROVED' ? 'bg-emerald-100 text-emerald-700' : 
                                entry.decision === 'REVIEW' ? 'bg-amber-100 text-amber-700' : 
